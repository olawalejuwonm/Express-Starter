...
import {
  IMongooseDocsSchema,
  IMongooseDocsSchemaField,
  TMongooseDocsFieldsObject,
} from '../types';
...
function parseField(path: SchemaType): IMongooseDocsSchemaField {
  ...
  if (type === 'Array') {
    if ((path as unknown as any).schema) {
      ...
      if (options.type[0] && typeof options.type[0].name) {
        nestedSchema = options.type[0].name;
      } else {
        nestedSchema = 'Not found';
      }
      //   nestedSchema = options.type[0]?.name;
    }
  } else if (
    ...
    fieldNames.forEach((fieldName: string) => {
      (nestedSchema as TMongooseDocsFieldsObject)[fieldName] = parseField(
        options.type.paths[fieldName],
      );
    });
  }
 ...
 * @param schema
 */
export function mongooseDocsSchemaJSON(
  schema: Schema,
): TMongooseDocsFieldsObject {
 ...
 */
export function mongooseDocsJSON(
  ...
  return mongooseModels.map((modelName: string) => {
    const schema: Schema = (mongooseInstance as unknown as any).model(
      modelName,
    ).schema;
